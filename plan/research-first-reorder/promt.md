...
- **Scope announcements**: Which functional modules, issue types, or code paths are explicitly declared in or out of vulnerability scope by the project.
- **Historical vulnerabilities**: For each finding, record: CVE ID (if any), date, affected module/component, vulnerability class, root cause, impact, severity, and attacker profile (attack vector, prerequisites, network position).
- **Vulnerability severity assessment guidance**: Explicit guidance from the project on how to assign severity levels to vulnerabilities (e.g., bug bounty severity tables, SECURITY.md severity criteria, or patterns observable from historical CVE severity assignments).
...
{Issue types that have historically been impactful for this project and have been identified as vulnerabilities in the past. Example:
...
- [ ] Security history researched (git log, CVEs, advisories; tier 3 sources if tier 1-2 were sparse)
