...
async def _run_module_agent(
    ...
    draft_dir = os.path.join(config.output_dir, "stage-3-details")

    if checkpoint.is_complete(key):
        logger.info("Stage 3: %s agent already complete.", module.id)
        return

    logger.info("Stage 3: Starting analysis of module %s.", module.id)
    ...
    for au_file in au_files:
        issues = validate_stage3_file(au_file)
        if not issues:
            continue
    ...
    logger.info("Stage 3: %s agent done.", module.id)
...
def _collect_already_renumbered(draft_dir: str) -> list[AnalysisUnit]:
    ...
    for path in au_files:
        try:
            ...
            logger.warning("Stage 3: Failed to read existing AU file %s: %s, skipping.", path, e)
            continue
...
def _collect_and_renumber(
    ...
    for module in modules:
        ...
        au_files = list_matching_files(draft_dir, pattern)
        if not au_files:
            logger.warning("Stage 3: No AU files found for %s.", module.id)
            continue
        ...
        for src_path in au_files:
            ...
            try:
                ...
                logger.warning("Stage 3: Failed to read %s: %s, skipping.", src_path, e)
                continue
            ...
            logger.info("Stage 3: %s -> %s (from %s)", os.path.basename(src_path), au_id, module.id)
...
async def run_stage3(
    ...
    stage2_output = os.path.join(config.output_dir, "stage-2-modules.json")
    draft_dir = os.path.join(config.output_dir, "stage-3-details")

    results = await run_parallel_limited(
        modules,
        config.max_parallel,
        lambda module, _: _run_module_agent(module, config, checkpoint, stage2_output),
    )

    for i, (status, _, error) in enumerate(results):
        if status == "rejected" and i < len(modules):
            logger.error("Stage 3: %s agent failed: %s", modules[i].id, error)
